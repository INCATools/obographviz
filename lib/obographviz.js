...
import CliqueIndex from './cliqueIndex.js';
...
class OboGraphViz {
     ...
     * 
     * @param {object} og - OBO Graph JSON object
     */
    constructor(og) {
        this._og = og;
        if (!og.graphs) {
     ...
     *
     * @param {string} dot - dot string (i.e. output of renderDot)
     * @param {(string|string[])} fmts - list of formats, or format singleton, e.g. ['png']
     */
    writeRenderedToFiles(dot, basePath, fmts) {
        var df = basePath + ".dot";
        writeFileSync(df, dot);
        if (typeof fmts === 'string') {
            fmts = [fmts];
        }
     ...
     *
     * @param {string[]} nestedRelations - list of object property URIs to be used as containment relations
     * @param {object} styleMap - stylemap
     * @param {string[]} highlightIds - list of node Ids to highlight
     * @returns {string} dot object
     */
    renderDot(nestedRelations, styleMap, highlightIds) {
        if (!styleMap) {
            styleMap = {};
        }
        if (!highlightIds) {
        ...
        const dotstr = dot.write(dotgraph);
        return dotstr;
    }
     ...
     *
     */
    createDigraph(nestedRelations, styleMap, highlightIds) {
        ...
        var dotgraph = new graphlib.Graph({ directed: true, compound: true, multigraph: true });
        ...
        if (styleMap.cliqueRelations) {
            ...
            if (styleMap.cliqueRelations.indexOf("equivalent_to") > -1) {
                ensl = ensl.concat(this._equivalentNodesSets);
            }
            if (styleMap.cliqueRelations.indexOf("xref") > -1) {
                for (let n of bg.all_nodes()) {
                    const xrefs = this.getXrefs(n);
        ...
        for (let i = 0; i < og.graphs.length; i++) {
            var g = og.graphs[i];
            ...
            var cgraph = new graphlib.Graph({ directed: true });

            if (cliques.length > 0) {
                const NPRED = '_in_equiv_clique';
                nestedRelationsDirect.push(NPRED);
                for (let c of cliques) {
                    const cid = '%CLIQUE:-' + c.join('-');
            ...
            var isClusterNode = function (id) {
                var isCluster = false;
                const cns = cgraph.predecessors(id);
            ...
            for (let j = 0; j < nodes.length; j++) {
                ...
                if (styleMap.displayAnnotations) {
                    ...
                    if (nodeForLabel.meta && nodeForLabel.meta.definition) {
                        const def = nodeForLabel.meta.definition;
                ...
                if (this.isSelected(n, styleMap)) {
                    ...
                    if (styleMap.prefixProperties) {
                        for (let k in styleMap.prefixProperties) {
                            let pfx = k;
                            // pfx should be of form "UBERON:"
                            if (!pfx.startsWith('http') && pfx.indexOf("/") == -1) {
                                pfx = pfx + ":";
                            }
                            if (nodeForLabel.id.startsWith(pfx)) {
                                const sm = styleMap.prefixProperties[k];
                                if (sm.fields) {
                    ...
                    if (styleMap.conditionalProperties) {
                        for (let k in styleMap.conditionalProperties) {
                            const cp = styleMap.conditionalProperties[k];
            ...
            for (let j = 0; j < edges.length; j++) {
                ...
                if (nestedRelationsDirect.indexOf(pred) > -1) {
                    ...
                    if (styleMap.relationProperties) {
                        const relStyles = styleMap.relationProperties;
                        if (relStyles[pred]) {
                    ...
                    for (let i in bpvs) {
                        const bpv = bpvs[i];
                        if (bpv.pred.startsWith('https://w3id.org/kgviz/')) {
                            const gvpred = bpv.pred.replace('https://w3id.org/kgviz/', '');
                            if (bpv.val != null) {
    ...
    isRedundant(s, o) {
        const bg = this._bbopg;
        for (let pe of bg.get_parent_edges(s)) {
            var p = pe.object_id();
            //console.log("CHK: "+s+" "+p);
            const asg = bg.get_ancestor_subgraph(p);
    ...
    getXrefs(bbop_node) {
        const md = bbop_node.metadata();
        if (md) {
            const xrefObjs = md.xrefs;
            if (xrefObjs) {
    ...
    isSelected(obj, styleMap) {
        ...
        const filter = styleMap.nodeFilter;
        if (!filter) {
            return true;
        }
        var isSelected = true;
...
function testConditions(node, conditionMap) {
    for (let p in conditionMap) {
        const expected = conditionMap[p];
        if (p == 'subset') {
            let inSubset = false
            if (node.meta && node.meta.subsets) {
                for (let k in node.meta.subsets) {
                    const s = node.meta.subsets[k]
...
const RESERVED = {'fields': true};

function addAllStyles(properties, styleMap) {
    if (styleMap) {
        for (let p in styleMap) {
            if (!RESERVED[p]) {
